...
module.exports = (db, io, v, socket) => async (
  jwt,
  userid,
  convoid,
  rawtext
  ...
  try {
    ...
    if (convo.length < 0)
      socket.emit('convo-message-response', 'ERROR: CONVO DOES NOT EXIST')
...
//       if (convo.length < 0)
//         this.emit('convo-message-response', 'ERROR: CONVO DOES NOT EXIST')
